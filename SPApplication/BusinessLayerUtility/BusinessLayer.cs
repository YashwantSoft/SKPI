...
namespace BusinessLayerUtility
{
    public class BusinessLayer
    {
        ...
        public void Connect()
        {
            ...
            DatabasePassword = BusinessResources.DATABASEPASSWORD;
            FormatPath = ApplicationPath + BusinessResources.FORMATPATH;
           // FormatPath =  BusinessResources.FORMATPATH;
            conString = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + DatabasePath + ";Jet OLEDB:Database Password=" + DatabasePassword + ";Persist Security Info=False;";
            objCon = new OleDbConnection(conString);
            objCon.Open();
        }

        public void FillComboBox_TableWise(ComboBox cmb, string TableName, string ValueMember, string DisplayMember)
        {
            try
            {
                cmb.DataSource = null;
                DataSet ds = new DataSet();
                Query = "select " + ValueMember + "," + DisplayMember + " from " + TableName + " where CancelTag=0 order by " + DisplayMember + "";
                ds = ReturnDataSet();
        ...
        public void FillCheckListBox(CheckedListBox cmb, string DisplayMember, string ValueMember)
        {
            try
            {
                DataSet ds = new DataSet();
        ...
        public void FillComboBox_DGV(DataGridViewComboBoxColumn cmb, string DisplayMember, string ValueMember)
        {
            try
            {
                DataSet ds = new DataSet();
