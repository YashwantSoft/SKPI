...
namespace SPApplication.Report
{
    public partial class CustomerDetails : Form
    {
        ...
        {
            InitializeComponent();
            objBL.Set_Report_Design(this, lblHeader, btnReport, btnClear, btnExit, BusinessResources.LBL_HEADER_CUSTOMERPAYMENTDEPOSITREPORT);
        ...
        private void ReceiptView()
        {
            ...
            if (cbToday.Checked)
                objBL.Query = "select S.CustomerId,Format([S.PaymentDate],'dd/MM/yyyy') as [Payment Date],C.CustomerName as [Customer Name],S.PaidAmount as [Paid Amount],S.TotalDue as [Total Due],S.PaymentMode,S.Naration,S.BankId,S.BankName,S.AccountNumber,S.TransactionDate,S.ChequeNumber from Receipt S inner join Customer C on C.ID=S.CustomerId where S.CancelTag=0 and C.CancelTag=0 and S.CustomerId=" + RedundancyLogics.CustomerId + " ";
            else
                objBL.Query = "select S.CustomerId,Format([S.PaymentDate],'dd/MM/yyyy') as [Payment Date],C.CustomerName as [Customer Name],S.PaidAmount as [Paid Amount],S.TotalDue as [Total Due],S.PaymentMode,S.Naration,S.BankId,S.BankName,S.AccountNumber,S.TransactionDate,S.ChequeNumber from Receipt S inner join Customer C on C.ID=S.CustomerId where S.CancelTag=0 and C.CancelTag=0 and S.CustomerId=" + RedundancyLogics.CustomerId + "  and S.PaymentDate between #" + dtpFromDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "# and #" + dtpToDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "#";
        ...
        private void SaleView()
        {
            ...
            if (cbToday.Checked)
            {
                if (cbSelectAll.Checked)
                    objBL.Query = "select S.CustomerId,S.ID as [Invoice No],Format([S.InvoiceDate],'dd/MM/yyyy') as [Invoice Date],C.CustomerName as [Customer Name],S.InvoiceTotal as [Invoice Total],S.PaymentMode as [Payment Type] from Sale S inner join Customer C on C.ID=S.CustomerId where S.CancelTag=0 and C.CancelTag=0 and S.CustomerId=" + RedundancyLogics.CustomerId + " and S.InvoiceDate BETWEEN " + "#" + dtpFromDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "# and #" + dtpToDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "# order by S.InvoiceDate desc"; 
                else
                    objBL.Query = "select S.CustomerId,S.ID as [Invoice No],Format([S.InvoiceDate],'dd/MM/yyyy') as [Invoice Date],C.CustomerName as [Customer Name],S.InvoiceTotal as [Invoice Total],S.PaymentMode as [Payment Type] from Sale S inner join Customer C on C.ID=S.CustomerId where S.CancelTag=0 and C.CancelTag=0 and S.ID=" + cmbBillNo.SelectedValue + " and S.CustomerId=" + RedundancyLogics.CustomerId + "  and S.InvoiceDate BETWEEN " + "#" + dtpFromDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "# and #" + dtpToDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "# order by S.InvoiceDate desc"; 
            }
            else
            {
                if (cbSelectAll.Checked)
                    objBL.Query = "select S.CustomerId,S.ID as [Invoice No],Format([S.InvoiceDate],'dd/MM/yyyy') as [Invoice Date],C.CustomerName as [Customer Name],S.InvoiceTotal as [Invoice Total],S.PaymentMode as [Payment Type] from Sale S inner join Customer C on C.ID=S.CustomerId where S.CancelTag=0 and C.CancelTag=0 and S.CustomerId=" + RedundancyLogics.CustomerId + " and  S.InvoiceDate  BETWEEN " + "#" + dtpFromDate.Value.ToString("dd/MM/yyyy") + "# and #" + dtpToDate.Value.ToString("dd/MM/yyyy") + "#";
                else
                    objBL.Query = "select S.CustomerId,S.ID as [Invoice No],Format([S.InvoiceDate],'dd/MM/yyyy') as [Invoice Date],C.CustomerName as [Customer Name],S.InvoiceTotal as [Invoice Total],S.PaymentMode as [Payment Type] from Sale S inner join Customer C on C.ID=S.CustomerId where S.CancelTag=0 and C.CancelTag=0 and S.ID=" + cmbBillNo.SelectedValue + " and S.CustomerId=" + RedundancyLogics.CustomerId + " and  S.InvoiceDate  BETWEEN " + "#" + dtpFromDate.Value.ToString("dd/MM/yyyy") + "# and #" + dtpToDate.Value.ToString("dd/MM/yyyy") + "#";
            }
        ...
        private void ExcelReport()
        {
            ...
            if (dgvSale.Rows.Count > 0)
            {
                ...
                objRL.Form_DestinationReportFilePath = "Customer Pending Payment\\" + RedundancyLogics.CustomerName + "\\";
                ...
                myExcelWorksheet.get_Range("A3", misValue).Formula = "Customer Name-" + RedundancyLogics.CustomerName.ToString();
