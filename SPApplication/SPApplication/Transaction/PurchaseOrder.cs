...
namespace SPApplication.Transaction
{
    public partial class PurchaseOrder : Form
    {
        ...
        {
            InitializeComponent();
            objDL.SetDesignMaster(this, lblHeader, btnSave, btnClear, btnDelete, btnExit, BusinessResources.LBL_HEADER_PURCHASEORDER);
        ...
        private void SetSupplierDetails()
        {
            ...
            if (SupplierId != 0)
            {
                ...
                txtSupplierDetails.Text = SupplierDetails.ToString();
                cmbType.Focus();
            }
        }
        ...
        {
            OtherMaterialFlag = 0;
            lblPreformDetails.Text = "Preform Details";
            objEP.Clear();
            TableID = 0;
            FlagDelete = false;
            Id = 0;
            ...
            objRL.Fill_Supplier_ListBox(lbSupplier, txtSupplierDetails.Text, "All");
            dgvCap.Rows.Clear();
            dgvWad.Rows.Clear();
            txtID.Text = "";
            txtFreight.Text = "";
        ...
        {
            int IDNo = 0;
            IDNo = Convert.ToInt32(objRL.ReturnMaxID("PurchaseOrder"));
        ...
        private void Fill_DataGridView_Products(DataGridView dgv, string ProductTypeSearch)
        {
            ...
            if (ProductTypeSearch == "Preform")
            {
                ColumnParameter = "P.PreformName";
                FetchColumn = "PreformName";
                JoinParameter = " Preform P on P.ID";
            ...
            else if (ProductTypeSearch == "Finish Goods" || ProductTypeSearch == "Other Material")
            {
                ColumnParameter = "M.MaterialName";
                FetchColumn = "MaterialName";
                JoinParameter = " OtherMaterial M on M.ID";
                WhereCondition = "M.CancelTag=0";
            }
            else
            {
            }
            ...
            objBL.Query = "select CRP.ID,CRP.PurchaseOrderId,CRP.PType,CRP.ProductId, " + ColumnParameter + ",CRP.Bags,CRP.PcsPerBag,CRP.Total from PurchaseOrderProducts CRP inner join " + JoinParameter + " =CRP.ProductId  where CRP.CancelTag=0  and CRP.PurchaseOrderId=" + TableID + " and CRP.PType='" + ProductTypeSearch + "' and " + WhereCondition + "";
            ds = objBL.ReturnDataSet();
            DataGridIndex = 0;

            if (ds.Tables[0].Rows.Count > 0)
            {
                if (!string.IsNullOrEmpty(Convert.ToString(ds.Tables[0].Rows[0]["PType"].ToString())))
                    ListType = Convert.ToString(ds.Tables[0].Rows[0]["PType"].ToString());

                if (ListType == "Other Material" || ListType =="Finish Goods")
                    OtherMaterialFlag = 1;
                else
                    OtherMaterialFlag = 0;
        ...
        private bool ValidationItem()
        {
            ...
            {
                lbSupplier.Focus();
                objEP.SetError(lbSupplier, "Select Supplier Name");
                return true;
            }
        ...
        private void Set_List()
        {
            if (cmbType.SelectedIndex > -1)
            {
                ...
                lblPreformDetails.Text = "Product Details";

                if (ListType == "Preform")
                    objRL.Fill_Preform_ListBox(lbList, txtSearch.Text, "All");
                else if (ListType == "Cap")
                    objRL.Fill_Cap_ListBox(lbList, txtSearch.Text, "All");
                else if (ListType == "Wad")
                    objRL.Fill_Wad_ListBox(lbList, txtSearch.Text, "All");
                else if (ListType == "Finish Goods")
                {
                    lblPreformDetails.Text = "Finish Goods";
                    objRL.Fill_OtherMaterial_ListBox_FG(lbList, txtSearch.Text, "All");
                }
                else
                {
                    lblPreformDetails.Text = "Other Material Details";
                    objRL.Fill_OtherMaterial_ListBox(lbList, txtSearch.Text, "All");
                }
            }
        }
        ...
        private void SetListSearchText()
        {
            if (cmbType.SelectedIndex > -1)
            {
                if (txtSearch.Text != "")
                {
                    ...
                    if (ListType == "Preform")
                        objRL.Fill_Preform_ListBox(lbList, txtSearch.Text, "Text");
                    else if (ListType == "Cap")
                        objRL.Fill_Cap_ListBox(lbList, txtSearch.Text, "Text");
                    else if (ListType == "Wad")
                        objRL.Fill_Wad_ListBox(lbList, txtSearch.Text, "Text");
                    else if (ListType == "Finish Goods")
                        objRL.Fill_OtherMaterial_ListBox_FG(lbList, txtSearch.Text, "Text");
                    else
                        objRL.Fill_OtherMaterial_ListBox(lbList, txtSearch.Text, "Text");
                }
            }
        }
        ...
        private void Fill_Details_Information()
        {
            if (cmbType.SelectedIndex > -1)
            {
                ...
                if (Id != 0)
                {
                    ...
                    if (ListType == "Preform")
                    {
                        objRL.Get_Preform_Records_By_Id(Id);
                        Details = objRL.PreformName;
                    }
                    else if (ListType == "Cap")
                    {
                    ...
                    {
                        objRL.Get_Material_Records_By_Id(Id);
                        Details = objRL.MaterialName;
                    }
        ...
        private bool Validation()
        {
            ...
            {
                lbSupplier.Focus();
                objEP.SetError(lbSupplier, "Select Supllier Name");
                return true;
            }
            //else if (txtID.Text == "")
            //{
        ...
        string PType = string.Empty, BagsInsert = string.Empty, PcsPerBagInsert = string.Empty, TotalInsert = string.Empty, MailStatus = string.Empty, POStatus = string.Empty;
        int ProductId = 0;
        ...
        protected void SaveDB()
        {
            if (!Validation())
            {
                ...
                if (TableID != 0)
                    if (FlagDelete)
                        objBL.Query = "update PurchaseOrder set CancelTag=1 where ID=" + TableID + "";
                    else
                        objBL.Query = "update PurchaseOrder set EntryDate='" + dtpDate.Value.ToShortDateString() + "',EntryTime='" + dtpTime.Value.ToShortTimeString() + "',SupplierId=" + SupplierId + ",Freight='" + txtFreight.Text + "',TermsOfDelivery='" + cmbTermsOfDelivery.Text + "',MailStatus='" + MailStatus + "',POStatus='" + POStatus + "',Naration='" + txtNaration.Text + "',UserId=" + BusinessLayer.UserId_Static + " where ID=" + TableID + "";
                else
                    objBL.Query = "insert into PurchaseOrder(EntryDate,EntryTime,SupplierId,Freight,TermsOfDelivery,MailStatus,POStatus,Naration,UserId) values('" + dtpDate.Value.ToShortDateString() + "','" + dtpTime.Value.ToShortTimeString() + "'," + SupplierId + ",'" + txtFreight.Text + "','" + cmbTermsOfDelivery.Text + "','" + MailStatus + "','" + POStatus + "','" + txtNaration.Text + "'," + BusinessLayer.UserId_Static + ")";
                ...
                if (Result > 0)
                {
                    if (TableID != 0)
                    {
                        objBL.Query = "delete from PurchaseOrderProducts where PurchaseOrderId=" + TableID + "";
                        objBL.Function_ExecuteNonQuery();
                    }
                    else
                        TableID = objRL.ReturnMaxID_Fix("PurchaseOrder","ID");
        ...
        private void SaveDB_Products(DataGridView dgv)
        {
            if (dgv.Rows.Count > 0)
            {
                for (int i = 0; i < dgv.Rows.Count; i++)
                {
                    ...
                    objBL.Query = "insert into PurchaseOrderProducts(PurchaseOrderId,PType,ProductId,Bags,PcsPerBag,Total) values(" + TableID + ", '" + PType + "'," + Id + ",'" + BagsInsert + "','" + PcsPerBagInsert + "','" + TotalInsert + "')";
                    objBL.Function_ExecuteNonQuery();
                }
            }
        }
        ...
        protected void FillGrid()
        {
            ...
            if (SearchTag)
                WhereClause = " and C.SupplierName like '%" + txtSearchName.Text + "%'";
            else if (IDFlag)
                WhereClause = " and CR.ID=" + txtSearchID.Text + "";
            else
                WhereClause = string.Empty;

            //Format(TIME(), "HH:nn:ss") AS CurrentTime24Hr
            //Format(TIME(), "hh:mm:ss AM/PM") AS CurrentTime12Hr;
            //"hh:mm:ss AM/PM"

            objBL.Query = "select CR.ID,CR.EntryDate as [Date],Format(CR.EntryTime, 'hh:mm AM/PM') as [Time],CR.SupplierId,C.SupplierName as [Supplier Name],CR.Freight,CR.TermsOfDelivery as [Terms Of Delivery],CR.MailStatus as [Mail Status],CR.POStatus as [PO Status],CR.Naration from PurchaseOrder CR inner join Supplier C on C.ID=CR.SupplierId where C.CancelTag=0 and CR.CancelTag=0 " + WhereClause + " order by CR.ID desc";
            //objBL.Query = "select CR.ID,CR.EntryDate as [Date],CR.EntryTime as [Time],CR.SupplierId,C.SupplierName as [Supplier Name],CR.Freight,CR.TermsOfDelivery as [Terms Of Delivery],CR.MailStatus as [Mail Status],CR.POStatus as [PO Status],CR.Naration from PurchaseOrder CR inner join Supplier C on C.ID=CR.SupplierId where C.CancelTag=0 and CR.CancelTag=0 " + WhereClause + " order by CR.EntryDate desc";
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                //3 CR.SupplierId,
                //4 C.SupplierName as [Supplier Name],
                //5 CR.Freight,
                //6 CR.TermsOfDelivery
                //7 CR.MailStatus  as [Mail Status]
                //8 CR.POStatus as [PO Status],
                //9 CR.Naration
        ...
        private void btnSaveInTally_Click(object sender, EventArgs e)
        {
            if (BusinessLayer.UserName_Static == BusinessResources.USER_ADMIN || BusinessLayer.UserName_Static == BusinessResources.USER_ACCOUNTS || BusinessLayer.UserName_Static == BusinessResources.USER_ACCOUNTS1)
            {
                ...
                {
                    DialogResult dr;
                    dr = objRL.ReturnDialogResult_PurchaseOrder();
                    if (dr == DialogResult.Yes)
                    {
                        POStatus = "Done";
                        objBL.Query = "update PurchaseOrder set POStatus='" + POStatus + "' where ID=" + TableID + "";
                        int Result = objBL.Function_ExecuteNonQuery();
                        if (Result > 0)
                        {
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (RowCount_Grid >= 0 && CurrentRowIndex >= 0)
                {
                    ...
                    SupplierId = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[3].Value);
                    SetSupplierDetails();
                    ...
                    if (!string.IsNullOrEmpty(Convert.ToString(dataGridView1.Rows[e.RowIndex].Cells[8].Value)))
                        POStatus = dataGridView1.Rows[e.RowIndex].Cells[8].Value.ToString();
                    if (!string.IsNullOrEmpty(Convert.ToString(dataGridView1.Rows[e.RowIndex].Cells[9].Value)))
                        txtNaration.Text = dataGridView1.Rows[e.RowIndex].Cells[9].Value.ToString();

                    //ListType = dataGridView1.Rows[e.RowIndex].Cells[8].Value.ToString();
                    
                    Fill_DataGridView_Products(dgvProduct, "Preform");
                    Fill_DataGridView_Products(dgvProduct, "Other Material");
                    Fill_DataGridView_Products(dgvProduct, "Finish Goods");
        ...
        private void SetReport()
        {
            using (new CursorWait())
            {
                ...
                objRL.Form_DestinationReportFilePath = "\\Purchase Order Requirements\\";
                objRL.Path_Comman();
                ...
                if(OtherMaterialFlag ==1)
                    myExcelWorksheet.get_Range("A2", misValue).Formula = " Purchase Order Preform, Caps, Wad Requirements";
                else
                    myExcelWorksheet.get_Range("A2", misValue).Formula = " Purchase Order Other Material Requirements";

                myExcelWorksheet.get_Range("A3", misValue).Formula = " Supplier Name";

                myExcelWorksheet.get_Range("C3", misValue).Formula = objRL.SupplierName;
                ...
                if (dgvProduct.Rows.Count > 0)
                {
                    ...
                    if(OtherMaterialFlag==0)
                        Fill_Merge_Cell("A", "J", misValue, myExcelWorksheet, "Preform");
                    else
                        Fill_Merge_Cell("A", "J", misValue, myExcelWorksheet, "Other Material");

                    RowCount++;
                    Fill_Merge_Cell("A", "A", misValue, myExcelWorksheet, "Sr.No.");

                    if (OtherMaterialFlag == 0)
                        Fill_Merge_Cell("B", "G", misValue, myExcelWorksheet, "Preform");
                    else
                        Fill_Merge_Cell("B", "G", misValue, myExcelWorksheet, "Other Material");
