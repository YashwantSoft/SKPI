...
{
    public partial class TemperatureReport : Form
    {
        ...
        bool FlagDelete = false, FlagToday = false;
        ...
        {
            InitializeComponent();
            objDL.Set_Report_Design(this, lblHeader, btnView,btnReport, btnClear, btnExit, BusinessResources.LBL_REPORT_EMPLOYEETEMPERATUREREPORT);
        ...
        private void TemperatureReport_Load(object sender, EventArgs e)
        {
            ClearAll();
        ...
        protected void FillGrid()
        {
            ...
            MainQuery = "select ET.ID,ET.EntryDate as [Date],ET.EntryTime as [Time],ET.Shift,ET.EmployeeId,E.FullName as [Employee Name],E.Designation,ET.InTime as [IN Time],ET.InTemperature as [IN Temp],ET.InRemarks as [In Remarks],ET.OutTime as [Out Time],ET.OutTemperature as [Out Temp],ET.OutRemarks as [Out Remarks]  from EmployeeTemperature ET inner join Employee E on E.ID=ET.EmployeeId where ET.CancelTag=0 and E.CancelTag=0";
            OrderByClause = "  order by ET.EntryDate asc";
            

            WhereClause = " and ET.EntryDate between #" + dtpFromDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "# and #" + dtpToDate.Value.ToString(RedundancyLogics.DateFormatMMDDYYYY) + "# ";
            
            if (!cbSelectAll.Checked)
                WhereClause = " and ET.EmployeeId=" + EmployeeId + "";
        ...
        private void ExcelReportMail()
        {
            ...
            objRL.isPDF = true;
            objRL.Form_ExcelFileName = "TemperatureReport.xlsx";
            objRL.Form_ReportFileName = "TemperatureReport-";
            objRL.Form_DestinationReportFilePath = "TemperatureReport\\";
            ...
            myExcelWorksheet.get_Range("G3", misValue).Formula = DateTime.Now.Date.ToString(BusinessResources.DATEFORMATDDMMYYYY);
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(dataGridView1.Rows[i].Cells[5].Value.ToString())))
                    Fill_Merge_Cell("C", "C", misValue, myExcelWorksheet, dataGridView1.Rows[i].Cells[5].Value.ToString());
                else
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(dataGridView1.Rows[i].Cells[3].Value.ToString())))
                    Fill_Merge_Cell("D", "D", misValue, myExcelWorksheet, dataGridView1.Rows[i].Cells[3].Value.ToString());
                else
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(dataGridView1.Rows[i].Cells[7].Value.ToString())))
                    Fill_Merge_Cell("E", "E", misValue, myExcelWorksheet, dataGridView1.Rows[i].Cells[7].Value.ToString());
                else
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(dataGridView1.Rows[i].Cells[8].Value.ToString())))
                    Fill_Merge_Cell("F", "F", misValue, myExcelWorksheet, dataGridView1.Rows[i].Cells[8].Value.ToString());
                else
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(dataGridView1.Rows[i].Cells[9].Value.ToString())))
                    Fill_Merge_Cell("G", "G", misValue, myExcelWorksheet, dataGridView1.Rows[i].Cells[9].Value.ToString());
                else
