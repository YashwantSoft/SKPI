...
namespace SPApplication.Settings
{
    public partial class EmailCredentials : Form
    {
        ...
        {
            InitializeComponent();
            objDL.SetDesign3Buttons(this, lblHeader, btnSave, btnClear, btnExit, BusinessResources.LBL_HEADER_EMAILCREDENTIALS);
            btnSave.Text = BusinessResources.BTN_SAVE;
        }

        private void btnSave_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(txtEmailAddress.Text)) && !string.IsNullOrEmpty(Convert.ToString(txtPassword.Text)))
            {
                if (TableID != 0)
